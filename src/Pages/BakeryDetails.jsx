...
export const BakeryDetails = () => {
    const { id } = useParams();
    const [bakry, setBakry] = useState([]);

    useEffect(() => {
        axios.get(`https://online-store-quc3.onrender.com/bakery/${id}`).then((res) => {
            setBakry(res.data);
        }).catch((err) => {
            console.log(err);
        })
    ...
    const { category, image, description, price_inr, title, discount_percentage, discount_price_inr, ETA } = bakry;
